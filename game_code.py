...
my_list = ["Red", "Orange", "White", "Black", "Green", "Blue", "Brown", "Yellow"]
timer = 60
score = 0
...
def startGame():
    ...
    if timer == 60:
        startCountDown()
        displayed_word_color = random.choice(my_list).lower()
        display_words.config(text=random.choice(my_list), fg=displayed_word_color)
...
start_button = Button(btn_frame, text="START", width=20, fg="yellow", bg="black", padx=20, pady=10,
                      command=startGame)
start_button.grid()

reset_button = Button(btn_frame, text="Reset", width=20, fg="black", bg="light blue", padx=20, pady=10,
                      command=resetGame)
